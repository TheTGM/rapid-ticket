 ...
 */
const sendMessage = async (messageBody, messageType) => {
  try {
    ...
    if (!queueUrl) {
      throw new Error(
        "URL de la cola SQS no configurada en variable de entorno QUEUE_URL"
      );
    }
    ...
    console.log(
      "[SQS] Cuerpo del mensaje (primeros 200 caracteres):",
      messageBodyStr.substring(0, 200) +
        (messageBodyStr.length > 200 ? "..." : "")
    );
    ...
    console.log(
      `[SQS] Mensaje enviado con éxito. MessageId: ${result.MessageId}`
    );
    return result;
  } catch (error) {

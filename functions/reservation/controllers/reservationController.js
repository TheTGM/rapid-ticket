...
const createReservation = async (req, res, next) => {
  try {
    ...
    try {
      const preliminaryCheck = await reservationModel.checkSeatsAvailabilityPreliminary(functionId, seatIds);
...
const checkReservationStatus = async (req, res, next) => {
  try {
    ...
    const reservation = await reservationModel.findReservationByTemporaryId(temporaryId);
    
    if (!reservation) {
      const isProcessing = await reservationModel.isReservationInProcessing(temporaryId);
...
const getReservation = async (req, res, next) => {
  try {
    ...
    if (reservation.status === 'pending') {
      timeLimit = await reservationModel.checkReservationTimeLimit(reservation.id);
    }
...
const confirmReservation = async (req, res, next) => {
  try {
    ...
    const originalReservation = await reservationModel.getReservationById(parseInt(id, 10));
    ...
    const timeLimit = await reservationModel.checkReservationTimeLimit(originalReservation.id);
    if (timeLimit?.isExpired) {
      await sqsService.sendMessage(
        { reservationId: originalReservation.id, reason: 'Tiempo de reserva expirado' },
        'EXPIRE_RESERVATION'
      );
...
const cancelReservation = async (req, res, next) => {
  try {
    ...
    if (originalReservation.status !== 'pending' && originalReservation.status !== 'confirmed') {
      return res.status(StatusCodes.BAD_REQUEST).json({
...
const checkReservationTime = async (req, res, next) => {
  try {
    ...
    const timeLimit = await reservationModel.checkReservationTimeLimit(parseInt(id, 10));
    ...
    if (timeLimit.isExpired) {
      await sqsService.sendMessage(
        { reservationId: parseInt(id, 10), reason: 'Tiempo de reserva expirado' },
        'EXPIRE_RESERVATION'
      );
    }
